...
@Client.on_message(
    filters.command(["play", f"play@{U}"]) & (filters.chat(CHAT) | filters.private) | filters.audio & filters.private)
async def yplay(_, message: Message):
    ...
    if type == "audio":
        if round(m_audio.audio.duration / 60) > DURATION_LIMIT:
            d = await message.reply_text(
                f"❌ Audios longer than {DURATION_LIMIT} minute(s) aren't allowed, "
                f"the provided audio is {round(m_audio.audio.duration / 60)} minute(s)")
            await mp.delete(d)
            await mp.delete(message)
            return
...
@Client.on_message(
    filters.command(["clearplaylist", f"clearplaylist@{U}"]) & admin_filter & (filters.chat(CHAT) | filters.private))
...
async def channel_play_list(client, m: Message):
    ...
    if len(m.command) < 2:
        k = await m.reply_text(
            ...
            '\n\n⚠ If you are using channel id, make sure both the bot and user account are member of the given '
            'channel.')
        await mp.delete(k)
        await mp.delete(m)
        return
        ...
        if " " in m.text:
            ...
            if ytplaylist == "peer":
                markup = InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton("🤖 GetPlayListBot",
                ...
                k = await m.reply(
                    "You Have Not Subscribed to MY Update Channel, "
                    "and Please Join My Update Channel to Use This Feature 🤒",
                    reply_markup=markup)
                await mp.delete(k)
                return
                ...
                k = await m.reply(
                    "The Url you gave is Invalid, It should be something like "
                    "<code>https://youtube.com/playlist?list=PL_rXc1ssylNebemAQVgDaOPijBaXU2gyD</code>")
                await mp.delete(k)
                return
...
async def upload(client, message):
    ...
    if playlist[0][3] == "telegram":
        await client.send_audio(chat_id=message.chat.id, audio=url,
                                caption=f"<b>Song: {playlist[0][1]}\nUploaded Using [MusicPlayer]("
                                        f"https://github.com/subinps/MusicPlayer)</b>")
...
admincmds = ["join", "unmute", "yplay", "mute", "leave", "clean", "vc", "pause", "resume", "stop", "skip", "radio",
             "stopradio", "replay", "restart", "volume", "shuffle", "clearplaylist", "cplay", "export", "import",
             f"export@{U}", f"import@{U}", f"yplay@{U}" f"cplay@{U}", f"clearplaylist@{U}", f"shuffle@{U}",
             f"volume@{U}", f"join@{U}", f"unmute@{U}", f"mute@{U}", f"leave@{U}", f"clean@{U}", f"vc@{U}",
             f"pause@{U}", f"resume@{U}", f"stop@{U}", f"skip@{U}", f"radio@{U}", f"stopradio@{U}", f"replay@{U}",
             f"restart@{U}"]
...
async def not_chat(_, m: Message):
    ...
    k = await m.reply(
        "<b>You can't use this bot in this group, for that you have to make your own bot from the [SOURCE CODE]("
        "https://github.com/subinps/MusicPlayer) below.</b>",
