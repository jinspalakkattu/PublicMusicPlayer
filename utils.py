...
RADIO_TITLE = os.environ.get("RADIO_TITLE", " 🎸 Music 24x7 | Radio Mode")
if RADIO_TITLE == "NO":
    RADIO_TITLE = None
...
class MusicPlayer(object):
    def __init__(self):
        self.group_call = GroupCallFactory(bot,
                                           GroupCallFactory.MTPROTO_CLIENT_TYPE.PYROGRAM).get_file_group_call()  # USER
    ...
    async def start_call(self):
        ...
        try:
            ...
            try:
                ...
                await bot.send(CreateGroupCall(  # USER
                    peer=(await bot.resolve_peer(CHAT)),  # USER
    ...
    async def c_play(self, channel):
        ...
        if channel.startswith("-100"):
            ...
            channel = channel
        try:
            chat = await bot.get_chat(channel)  # USER
            print("Starting Playlist from", chat.title)
            async for m in bot.search_messages(chat_id=channel, filter="audio", limit=LIMIT):  # USER
                ...
                if round(m_audio.audio.duration / 60) > DURATION_LIMIT:
                    ...
                    data = {1: m_audio.audio.title, 2: m_audio.audio.file_id, 3: "telegram",
                            4: f"[{chat.title}]({m_audio.link})", 5: f"{nyav}_{m.message_id}"}
    ...
    async def get_playlist(self, user, url):
        ...
        client = group_call.client
        try:
            k = await bot.send_message(chat_id="GetPlayListBot", text="/start")  # USER
        ...
        if Config.DELETE_HISTORY:
            try:
                await bot.send(DeleteHistory(peer=(await bot.resolve_peer("GetPlayListBot")),
                                             max_id=0, revoke=True))  # USER
            except:
                pass
        if MAX == 0:
            return 'timeout'
